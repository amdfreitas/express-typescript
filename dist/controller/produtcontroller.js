...
const modelproduto_1 = __importDefault(require("../model/modelproduto"));
class ProndutoController {
    ...
    async produtCreate(req, resp) {
        ...
        const produt = new modelproduto_1.default(id, nome, preco, descrip);
        await produtrepost_1.ProdutoModel.create(produt)
            .then(data => resp.status(201).json(data))
            .catch(error => console.log(error));
    }
    async pesquesid(req, resp) {
        const id = req.params.id;
        await produtrepost_1.ProdutoModel.findOne({
            where: {
                "id": id
            }
        })
